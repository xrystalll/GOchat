...
for(i=0;i<elements.length;i++)!function(e){
	...
	usernameInput.on("keyup",function(l){
		if(13===l.keyCode&&e.value.length>0){
...
messages.limitToLast(69).on("child_added", function(i) {
	wrap.append('<div class="msb"><div  onclick="DoSmilie(&#39;' + $.sanitize(i.val().user) + ', &#39;);" class="cover" style="background-image: url(' + $.sanitize(i.val().image) + ');"></div><div class="cover_n">' + $.sanitize(i.val().user) + "</div><span>" + $.sanitize(i.val().user) + "</span> <time>" + $.sanitize(i.val().time) + "</time><div>" + $.sanitize(i.val().message) + "</div></div>"),
	window.scrollTo(0,document.body.scrollHeight);
	var s = new Audio;
	return s.src="src/sound/new_in.wav",
	void s.play()
	//<div class="empty ic"><div class="empty_words">Сообщений пока нет!</div></div>
});
