...
messages.limitToLast(100).on("child_added", function(snap) {
	wrap.append('<div class="msb"><div  onclick="DoSmilie(&#39;' + $.sanitize(snap.val().user) + ',&#39;);" class="cover" style="z-index: 2; background-image: url(' + $.sanitize(snap.val().image) + ');"></div><div class="cover_n">' + $.sanitize(snap.val().user) + '</div><span>' + $.sanitize(snap.val().user) + '</span> <time>' + $.sanitize(snap.val().time) + '</time><div>' + $.sanitize(snap.val().message) + '</div></div>');
	window.scrollTo(0,document.body.scrollHeight);
	var audio = new Audio();
